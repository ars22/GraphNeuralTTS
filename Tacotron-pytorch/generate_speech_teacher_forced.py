...
def generate_speech(args):
    ...
    with torch.no_grad():
        for batch_i, (id, graphs, txt_lengths, ground_truth_mel, _, add_info) in enumerate(inference_dataloader):
            if generated == args.top_k and args.top_k != -1:
                break
            ...
            ground_truth_mel = ground_truth_mel[indices]
            ground_truth_mel = ground_truth_mel.to(device=torch.device('cuda') if args.gpu else torch.device('cpu'))
            
            mel, spec, attn = model(graphs, text_lengths=sorted_lengths, add_info=add_info, melspec=ground_truth_mel)
            # Generate wav file
